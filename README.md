...
### Our Solution
-   **Atomic Permissions:** Granular control at the "action" level (e.g., `view_leads` is separate from `delete_leads`).
-   **Visual Admin Control:** Managers can toggle permissions for their teams without any technical knowledge.
-   **Hierarchical Safety:** A built-in fail-safe prevents users from granting more access than they personally hold.
...
| **Layer** | **Technologies** |
| :--- | :--- |
| **Frontend** | Next.js 16 (App Router), TypeScript, Tailwind CSS v4, Shadcn/UI, Redux Toolkit |
| **Backend** | Node.js, Express, MongoDB + Mongoose, Redis (High-speed caching) |
| **Security** | JWT (Dual-Token Rotation), HTTP-only Cookies, Zod Validation, Rate Limiting |
| **Real-time** | Socket.IO (Bidirectional Communication) |
| **Utilities** | Winston (Logging), PDFKit (Reporting), Multer (Storage), Nodemailer (Email) |
...
├── server/          # Enterprise Express backend with modular domain logic
└── README.md        # Documentation & Value Proposition
```
